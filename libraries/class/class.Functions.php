...
class Functions
{
	...
	{
		if (count($arrayDomain) == 0 && $http == 'http://') {
...
*/
	...
	public function listsGallery($file = '')
	{
		...
		if (!empty($file) && !empty($_FILES[$file]['name'])) {
			$fileLists = str_replace('"', '', implode(',',$_FILES[$file]['name']));
	...
	public function createThumb($width_thumb = 0, $height_thumb = 0, $zoom_crop = '1', $src = '', $watermark = null, $path = THUMBS, $preview = false, $args = array(), $quality = 100)
	{
		...
		if (!empty($watermark['status']) && $watermark['status'] == 'hienthi') {
...
//die('aaa');
	...
	public function sale($regular_price = 0,$sale_price = 0)
	{
		if ($sale_price > 0) {
			$kq = '<div class="gia-giam-d"> <span> Giảm ' . $this->formatMoney(round(($regular_price - $sale_price), 0)) . '</span> </div>'; 
			return $kq;
		}
	}
