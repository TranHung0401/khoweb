...
if ($id != '') {
	/* Lấy sản phẩm detail */
	$rowDetail = $d->rawQueryOne("select iframevd$lang,type, id, name$lang, slugvi, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, discount, sale_price, regular_price, thongtin$lang from #_product where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));

	$tintuc = $d->rawQuery("select id, name$lang, slugvi, slugen, photo, date_created from #_news where type='kinh-nghiem' and find_in_set('hienthi',status) and find_in_set('noibat',status) order by numb,id desc");
	...
	/* Lấy cấp 1 */
	$productList = $d->rawQueryOne("select id, name$lang, slugvi, slugen, type from #_product_list where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($rowDetail['id_list'], $type));
	...
	if(!empty($_POST['submit-option']))
    {
        $data = array();
        $data['fullname'] = htmlspecialchars($_POST['ten']);
        $data['phone'] = htmlspecialchars($_POST['dienthoai']);
	...
	/* Lấy cấp 1 detail */
	$productList = $d->rawQueryOne("select id, name$lang, descvi,slugvi, slugen, type, photo, options from #_product_list where id = ? and type = ? limit 0,1", array($idl, $type));
	...
	$startpoint = ($curPage * $perPage) - $perPage;
	$limit = " limit " . $startpoint . "," . $perPage;
	$sql = "select photo, name$lang, slugvi,descvi, slugen, sale_price, regular_price, discount, id, type from #_product where $where order by numb,id desc $limit";
	$product = $d->rawQuery($sql, $params);
	$sqlNum = "select count(*) as 'num' from #_product where $where order by numb,id desc";
	$count = $d->rawQueryOne($sqlNum, $params);
	$total = (!empty($count)) ? $count['num'] : 0;
	...
	/* Lấy cấp 1 */
	$productList = $d->rawQueryOne("select id, name$lang, descvi,slugvi, slugen from #_product_list where id = ? and type = ? limit 0,1", array($productItem['id_list'], $type));
	...
	/* Lấy brand detail */
	$productBrand = $d->rawQueryOne("select name$lang, slugvi,descvi ,slugen, id, type, photo, options from #_product_brand where id = ? and type = ? limit 0,1", array($idb, $type));
	...
	$startpoint = ($curPage * $perPage) - $perPage;
	$limit = " limit " . $startpoint . "," . $perPage;
	$sql = "select photo, name$lang, slugvi, slugen,descvi, sale_price, regular_price, discount, id, type from #_product where $where order by numb,id desc $limit";
	$product = $d->rawQuery($sql, $params);
	$sqlNum = "select count(*) as 'num' from #_product where $where order by numb,id desc";
	$count = $d->rawQueryOne($sqlNum, $params);
	$total = (!empty($count)) ? $count['num'] : 0;
